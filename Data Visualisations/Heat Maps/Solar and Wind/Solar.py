...
solar_path = r"C:\Users\archi\Data Visualisations\Heat Maps\Solar and Wind\Solar.xlsx"

# === Load solar data ===
solar_data = pd.read_excel(solar_path)
solar_data.columns = ['province', 'solar_capacity_MW']
solar_data['province'] = solar_data['province'].str.strip()
...
# Map names and merge
solar_data['mapped_name'] = solar_data['province'].map(province_name_map)
china_map = gpd.read_file(shapefile_path)
china_map = china_map.merge(solar_data, how='left', left_on='name', right_on='mapped_name')

# === Plot heat map ===
fig, ax = plt.subplots(figsize=(10, 12))
plot = china_map.plot(column='solar_capacity_MW',
                      cmap='YlOrBr',
                      linewidth=0.5,
                      edgecolor='black',
                      legend=True,
...
for idx, row in china_map.iterrows():
    if pd.notnull(row['solar_capacity_MW']):
...
# ax.set_title("Prospective Solar Capacity by Province (MW)", fontsize=14)
ax.axis('off')
plt.tight_layout()
plt.show()