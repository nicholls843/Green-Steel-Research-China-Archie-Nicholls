...
file_path = r"C:\Users\archi\Data Visualisations\LCOS ETS\LCOS ETS.csv"
...
for col in ["City", "Province"]:
    if col in df.columns:
        df[col] = (
            df[col].astype(str)
                  .str.replace("\u0092", "'", regex=False)  # bad control char from cp1252
...
# --- Distinct line styles for ETS central lines ---
line_style_map = {"Current": "-", "2030": "--", "2040": "-.", "2050": ":"}

# --- BF–BOF baseline and CO2 intensity ---
bf_bof_base = 539.00
co2_intensity = 2.1  # tCO2 per tonne steel (BF–BOF)

# --- Central China ETS-adjusted BF–BOF costs by year (USD/t steel) ---
china_ets_central = {
    "Current": bf_bof_base + 8.59  * co2_intensity,
    "2030":   bf_bof_base + 19.36 * co2_intensity,
    "2040":   bf_bof_base + 35.06 * co2_intensity,
    "2050":   bf_bof_base + 50.61 * co2_intensity,
}

# --- Use a uniform ±margin per tCO2 to create a band around each central line ---
uniform_margin_per_tco2 = 3.0  # USD per tCO₂
range_facecolor = "lightgray"
range_alpha = 0.30  # a touch darker

# === 1) SCATTER with COMPETITIVE RINGS ===
for scenario in ["1", "2", "3"]:
    ...
    for year, marker in marker_map.items():
        ...
        # Which are competitive vs ETS threshold this year?
        thr = china_ets_central.get(year, np.nan)
        ...
        # overlay RINGS to highlight competitive points
        if comp_mask.any():
    ...
    # ETS bands + patterned central lines (+$ in legend labels)
    ets_line_handles = []
    half_width = co2_intensity * uniform_margin_per_tco2  # e.g., 2.1 * 3.0 = 6.3 $/t steel
    for year in year_order:
        if year not in china_ets_central:
            continue
        central = float(china_ets_central[year])
        ...
        ets_line_handles.append(
            mlines.Line2D([], [], color="dimgray", linestyle=ls, linewidth=1.8,
                          label=f"China ETS {year} (${central:,.1f})")
        )
    ...
    band_handle = mpatches.Patch(facecolor=range_facecolor, alpha=range_alpha,
                                 label=f"ETS range (±{uniform_margin_per_tco2:.1f} $/tCO₂ ≈ ±${half_width:.1f}/t steel)")
    ax.legend(handles=[band_handle, *ets_line_handles],
              title="ETS reference", loc="upper left", bbox_to_anchor=(1.23, 0.66))
    ...
    plt.savefig(f"LCOS_Scenario_{scenario}_ChinaETS_scatter.png", dpi=300, bbox_inches='tight')
    plt.show()

# === 2) COMPETITIVENESS HEATMAP with gold star + circle markers ===
for scenario in ["1", "2", "3"]:
    df_s = long_df[long_df["Scenario"] == scenario].copy()
    if df_s.empty:
        continue

    # compute delta = LCOS - ETS threshold for that row's year
    df_s["ETS_threshold"] = df_s["Year"].astype(str).map(china_ets_central).astype(float)
    df_s["Delta_vs_ETS"] = df_s["Cost"].astype(float) - df_s["ETS_threshold"]

    # pivot to City x Year matrix (Δ values)
    pivot = df_s.pivot_table(index="City", columns="Year",
                             values="Delta_vs_ETS", aggfunc="mean")
    pivot = pivot.reindex(columns=year_order)

    # sort cities: earliest competitive year first, then by 2050 delta
    earliest_idx = []
    ...
    cmap.set_bad(color="#dddddd", alpha=0.6)  # show missing as light grey
    ...
    ax.set_title(f"Competitiveness vs ETS (Δ = LCOS − ETS threshold) — Scenario {scenario}")
    ax.set_xlabel("Year")
    ax.set_ylabel("City")

    # colorbar
    cbar = fig.colorbar(im, ax=ax)
    cbar.set_label("Δ cost vs ETS (USD/t)\n(negative = competitive)")

    # --- Overlay markers per cell: gold star for competitive, circle for in-band-above-central ---
    half_width = co2_intensity * uniform_margin_per_tco2  # constant band half-width in $/t steel

    # Pre-compute mean LCOS per (City, Year) for decision logic
    mean_lcos = (df_s.groupby(["City", "Year"])["Cost"]
                   .mean()
    ...
    for i, city in enumerate(pivot.index):
        for j, year in enumerate(year_order):
            ...
            lcos = mean_lcos.get((city, year), np.nan)
            central = china_ets_central.get(year, np.nan)
            if pd.isna(lcos) or pd.isna(central):
                continue

            # Competitive → gold star
            if lcos <= central:
                ax.scatter(j, i, marker="*", s=95, edgecolors="k",
                           facecolors="gold", linewidths=0.6, zorder=4)
            # In band but above central → circle
            elif (lcos > central) and (lcos <= central + half_width):
                ax.scatter(j, i, marker="o", s=42, facecolors="none",
                           edgecolors="k", linewidths=0.9, zorder=3)
            # else: above band -> no marker
    ...
    fig.tight_layout()
    plt.savefig(f"LCOS_Scenario_{scenario}_ChinaETS_heatmap.png", dpi=300, bbox_inches='tight')
    plt.show()

    # (optional) export summary of first competitive year per city
    summary = []
    for city in pivot.index:
        row = pivot.loc[city]
        first_year = next((y for y in year_order
                           if y in row.index and pd.notna(row[y]) and row[y] <= 0), None)
        summary.append({"City": city, "FirstCompetitiveYear": first_year})
    pd.DataFrame(summary).to_csv(f"Competitiveness_S{scenario}_summary.csv", index=False)
